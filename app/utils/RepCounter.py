...
class RepCounter(object):
    ...
    def shoulderpressCounter(self,keypoints):
        ...
        if "dual" in self.exercise:
            ...
            leftAngle = Pose.calculate_angle(leftShoulder, leftElbow, leftWrist)
            rightAngle = Pose.calculate_angle(rightShoulder, rightElbow, rightWrist)
            if rightAngle>=90 and leftAngle>=90 :
                self.state="stage"
                ...
                if self.state=="up":
                    ...
                    self.state="mid-up"
            elif rightAngle<=150 and leftAngle<=150 and self.state=="mid-up":
                self.state="up"
                self.reps+=1
        elif "left" in self.exercise:
            leftWrist=keyPoints[15][:2]
            ...
            if leftAngle>=90 :
                self.state="stage"
            elif leftAngle<90 and leftAngle>150:
                if self.state=="down":
                    self.state="mid-down"
                else:
                    self.state="mid-up"
            elif leftAngle<=150 and self.state=="mid-up":
                self.state="up"
                self.reps+=1
        elif "right" in self.exercise:
            rightWrist=keyPoints[16][:2]
            ...
            if rightAngle>=90:
                self.state="stage"
            elif rightAngle<90 and rightAngle>150 :
                if self.state=="down":
                    self.state="mid-down"
                else:
                    self.state="mid-up"
            elif rightAngle<=150 and self.state=="mid-up":
                self.state="up"
                self.reps+=1
        return self.reps
